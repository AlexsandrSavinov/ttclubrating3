...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли СоцСеть = Перечисления.СоцСеть.Телеграмм Тогда 
		...
		Элементы.Бот.Видимость = Не Константы.ИспользоватьБотаTtclubrating.Получить();
	КонецЕсли; 
	...
	Если Не ЗначениеЗаполнено(СсылкаНаДокумент) Тогда
		ЗаголовокДляДокумента = "Документ от "+ КР_СерверныеСервер.ДатаСеанса();
	Иначе
...
Функция ПолучитьДанныеДляОтправкиНаСервере(Бот)
	...
	Если Константы.ИспользоватьБотаTtclubrating.Получить() Тогда
		...
		ПараметрыВозврата.Вставить("ТокенБота", КР_РаботаССоциальнымиСетямиКлиентСервер.ПолучитьТокенБотаТТ().Токен);
		Если ПараметрыВозврата.ТокенБота = "" Тогда
			ПараметрыВозврата.ТокенБота 	= КР_СерверныеСервер.ЗначениеРеквизитаИзСсылки(Бот, "Токен");
			ПараметрыВозврата.Бот				= Бот;
		КонецЕсли; 
	Иначе
		ПараметрыВозврата.Вставить("Бот", Бот);
		ПараметрыВозврата.Вставить("ТокенБота", КР_СерверныеСервер.ЗначениеРеквизитаИзСсылки(Бот, "Токен"));
	КонецЕсли; 
