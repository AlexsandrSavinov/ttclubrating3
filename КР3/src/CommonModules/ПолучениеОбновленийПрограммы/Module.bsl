...
Процедура ПолучитьОписаниеФайловОбновленияКонфигурацииИПлатформы(
	...
	ПараметрыОтправки.Вставить("ФорматДанныхДляОбработки", 1);
	ПараметрыОтправки.Вставить("Таймаут"                 , 30);
	URLОперации = URLОперации + ОбновлениеКонфигурации.ИдентификаторВерсии;
	РезультатОтправки = ИнтернетПоддержкаПользователей.ЗагрузитьСодержимоеИзИнтернет(
		URLОперации,
		,
		,
...
//Наименование
//Описание
//ОписаниеИзмененныхМетаданных
//Отозвано
//Применимость
//Размер
	// Обработка ответа.
	Попытка //TODO: JSON для исправлений понять структуру {"errorName":null,"errorMessage":null,"patchUpdateList":[]}
