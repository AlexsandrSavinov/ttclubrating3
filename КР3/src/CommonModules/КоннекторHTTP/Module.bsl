...
#Если МобильноеПриложениеСервер Тогда
	ВызватьИсключение(НСтр("ru='Работа с Zip-файлами в мобильной платформе не поддерживается';en='Working with Zip files is not supported in the mobile platform'"));
#Иначе
...
Процедура Приостановить(ДлительностьОстановкиВСекундах)
	...
	Пока ТекущаяДата < ЖдатьДо Цикл
		...
		Если РеальныйТаймаут < МинимальныйТаймаутВМиллисекундах Тогда
			ВызватьИсключение(НСтр("ru='Процедура Приостановить не работает должным образом';en='The Suspend procedure does not work properly'"));
		КонецЕсли;
		ТекущаяДата = ТекущаяУниверсальнаяДата();
	КонецЦикла;
...
Функция РаскодироватьСтрокуНаСервере(парСтр, парСпособКодированияСтроки = Неопределено) Экспорт

	Если парСпособКодированияСтроки = Неопределено Тогда
		парКодирование = СпособКодированияСтроки.КодировкаURL
	Иначе
		парКодирование = парСпособКодированияСтроки;
	КонецЕсли; 
	
	Возврат РаскодироватьСтроку(парСтр, парКодирование);
...
Функция ЗначениеВJSON(Значение) Экспорт
	...
	ЗаписатьJSON(ЗаписьJSON, Значение, , "ПреобразоватьЗначениеДляJSON", КоннекторHTTP);
	Возврат ЗаписьJSON.Закрыть();
...
////////////////////////////////////////////////////////////////////////////////
//
// Функция ПреобразоватьЗначениеДляJSON
//
// Описание:
//
//
// Параметры:
// парЗначение - Любой - значение допустимое для формирования JSON
//
// Возвращаемое значение:  
//
Функция ПреобразоватьЗначениеДляJSON(Свойство = "", парЗначение, ДопПараметры = Неопределено, Отказ = Ложь) Экспорт
	
	Если ТипЗнч(парЗначение) = Тип("Число")
		Или ТипЗнч(парЗначение) = Тип("Строка")
		Или ТипЗнч(парЗначение) = Тип("Булево")
		Или ТипЗнч(парЗначение) = Тип("Дата") Тогда
		Возврат парЗначение;
	ИначеЕсли ТипЗнч(парЗначение) = Тип("Null") Тогда 
		Возврат "";
	Иначе
		Возврат Строка(парЗначение);
	КонецЕсли; 
