...
Функция ПроверитьДоступностьТелеграмм(ТокенТелеграмм = "", Соединение = Неопределено) Экспорт
	...
	Если Ответ.КодСостояния = 200 Тогда
		СтруктураОтвета = КР_ОбщийКлиентСервер.СформироватьОтветHTTPВСтруктуру(Ответ);
		СтрокаОтвета = "";
...
Функция ПолучитьТокенБотаТТ() Экспорт
	...
	Попытка		
		ОтветHTTP = КоннекторHTTP.Get(КР_СерверныеСервер.ИнформацияОКонфигурации().АдресИнформацииОКонфигурации + "/updates/bot_tg.txt");
		Если ОтветHTTP.КодСостояния = 200 Тогда
			Стр = СокрЛП(КоннекторHTTP.КакТекст(ОтветHTTP));
		Иначе
			Стр = "";
		КонецЕсли; 
