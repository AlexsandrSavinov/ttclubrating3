...
Функция ПолучитьФормулыДляРасчетаРейтинга(ДатаФормул = Неопределено) Экспорт

	Если ДатаФормул = Неопределено Тогда
		ДатаФормул = ДатаСеанса();
	КонецЕсли;
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   |	ФормулыРасчетаРейтингаСрезПоследних.ТипПрироста
				   |ИЗ
				   |	РегистрСведений.ФормулыРасчетаРейтинга.СрезПоследних(&ТекДата,) КАК ФормулыРасчетаРейтингаСрезПоследних";
	Запрос.УстановитьПараметр("ТекДата", ДатаФормул);
	Результат = Запрос.Выполнить().Выгрузить();
	
	// у нас всегда будут какие то значения, так как при запуске программы проверяются формулы
	ДанныеПоФормулам = Новый Соответствие;
	Для Каждого ТекСтр Из Результат Цикл
		ДанныеПоФормулам.Вставить(ТекСтр.ТипПрироста, ТекСтр.Формула);
	КонецЦикла;

	Если ДанныеПоФормулам.Количество() = 0 Тогда
		ДанныеПоФормулам.Вставить(Перечисления.ТипыПриростов.ПриростВыигравшего, "(100-(РТв-РТп))/10");
		ДанныеПоФормулам.Вставить(Перечисления.ТипыПриростов.ПриростПроигравшего, " -(100-(РТв-РТп))/20");
	КонецЕсли;

	Возврат ДанныеПоФормулам;
...
Функция ПолучитьТекущийРейтингУчастника(Участник, Дата = Неопределено, ВидРейтинга = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ВидРейтинга", ?(ВидРейтинга = Неопределено, Справочники.ВидыРейтинга.ПустаяСсылка(),
		ВидРейтинга));
	Результат = Запрос.Выполнить().Выгрузить();
	Если Результат.Количество() > 0 Тогда
...
Процедура ВыполнитьПроцедурыИФункцииПриНачалеРаботыСистемы() Экспорт
...
Функция ПроверкаВерсииПлатформы() Экспорт
	...
	Попытка
		Если Число(МассивДляПроверкиТекущей[0]) >= Число(МассивДляПроверкиМинимальной[0]) И Число(
			МассивДляПроверкиТекущей[1]) >= Число(МассивДляПроверкиМинимальной[1]) И Число(МассивДляПроверкиТекущей[2])
			>= Число(МассивДляПроверкиМинимальной[2]) Тогда
...
// 
// Возвращаемое значение:
//  Структура - Информация о конфигурации:
// * Имя - Строка -
// * Синоним - Строка -
...
Функция ИнформацияОКонфигурации() Экспорт
	...
	ИнформацияОКонфигурации.Вставить("Имя", Метаданные.Имя);
	ИнформацияОКонфигурации.Вставить("Синоним", Метаданные.Синоним);
	ИнформацияОКонфигурации.Вставить("АдресИнформацииОКонфигурации", Метаданные.АдресИнформацииОКонфигурации);
