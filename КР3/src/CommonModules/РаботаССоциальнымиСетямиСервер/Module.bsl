...
Функция АктивныеБоты() Экспорт
	...
	|	И ТелеграммБоты.Токен <> """"";
	 ...
	 Возврат КР_СерверныеСервер.ТаблицаЗначенийВМассив(Результат);
	...
	Если Константы.ИспользоватьПроксиДляТелеграмма.Получить() Тогда
		...
		Если РезультатБоты.Количество() > 0 Тогда
			...
			Для каждого ТекСервер Из СписокПроксиСерверов Цикл
				...
				мИнтернетПрокси = КР_РаботаССоциальнымиСетямиКлиентСервер.СоздатьИнтернетПрокси(ТекСервер);
				Соединение = КР_РаботаССоциальнымиСетямиКлиентСервер.УстановитьСоединениеСТелеграмм(мИнтернетПрокси, 5);
				
				Попытка
					ОтветБота = КР_РаботаССоциальнымиСетямиКлиентСервер.ВыполнитьКомандуТелеграммБота(ТекБот, Соединение);
