...
Процедура ОткрытьФормуОтправкиВСоцСеть(ИсточникВызова, ТипСоцСети) Экспорт
	...
	ОповещениеПослеЗакрытия = Новый ОписаниеОповещения("ПослеЗакрытияФормыОтправки", КР_РаботаССоциальнымиСетямиКлиент,
		ПараметрыОтправки);
...
Процедура ПослеЗакрытияФормыОтправки(Результат, ДопПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		Результат.ТабДокДляОтправки.НачатьЗапись(Новый ОписаниеОповещения("ПослеЗаписиФайла",
			КР_РаботаССоциальнымиСетямиКлиент, Результат), Результат.втИмяФайла,
			ТипФайлаТабличногоДокумента[Результат.Формат]);
...
Процедура КонвертироватьHTMLвPNG(ПараметрыДляОтправки)
	...
	НачатьЗапускПриложения(Новый ОписаниеОповещения("ПослеЗапускаПриложенияКонвертирования",КР_РаботаССоциальнымиСетямиКлиент,ПараметрыДляОтправки),ПараметрыДляОтправки.СтрокаКонвертирования,,Истина);
...
Процедура ОтправитьДокументыВоВконтакте(ДанныеДляОтправки) Экспорт
	...
	Если ДанныеДляОтправки.ИДДляОтправки.Количество() = 0 Тогда
		...
		Сообщение.Сообщить(); 
		//можно удалить файл
		НачатьУдалениеФайлов(Новый ОписаниеОповещения("ПослеУдаленияФайлов",КР_РаботаССоциальнымиСетямиКлиент),ПутьКФайлу);
		Возврат;
	КонецЕсли; 
	...
	Для Каждого ТекЗапись Из ДанныеДляОтправки.ИДДляОтправки Цикл
		...
		СтруктураДанных = КР_ОбщийКлиентСервер.СформироватьОтветHTTPВСтруктуру(Ответ);	//считывание в структуру			
		...
		Если Ответ.КодСостояния = 200 Тогда
			Если СтруктураДанных.Свойство("response") Тогда
				...
				СтруктураАдреса = КР_КлиентскиеКлиент.СтруктураURI(АдресДляЗагрузкиФайла);
				...
				ПараметрыОтвета = КР_ОбщийКлиентСервер.СформироватьОтветHTTPВСтруктуру(ОтветHTTP);
				...
				Если ПараметрыОтвета.Свойство("file") Тогда
					...
					РезультатПубликации = КР_ОбщийКлиентСервер.СформироватьОтветHTTPВСтруктуру(ОтветНаЗапрос);
					...
					Если РезультатПубликации.Свойство("response") Тогда
						...
						РезультатОтправки = КР_ОбщийКлиентСервер.СформироватьОтветHTTPВСтруктуру(ОтветНаРазмещениеЗаписи);
						...
						Если ДанныеДляОтправки.Свойство("втИмяФайла") Тогда
							Если ЗначениеЗаполнено(ДанныеДляОтправки.втИмяФайла) Тогда
								НачатьУдалениеФайлов(Новый ОписаниеОповещения("ПослеУдаленияФайлов",КР_РаботаССоциальнымиСетямиКлиент),ДанныеДляОтправки.втИмяФайла);
							КонецЕсли; 
						КонецЕсли; 
						Если ДанныеДляОтправки.Свойство("ФайлКартинки") Тогда
							Если ЗначениеЗаполнено(ДанныеДляОтправки.ФайлКартинки) Тогда
								НачатьУдалениеФайлов(Новый ОписаниеОповещения("ПослеУдаленияФайлов",КР_РаботаССоциальнымиСетямиКлиент),ДанныеДляОтправки.ФайлКартинки);
							КонецЕсли; 
						КонецЕсли; 
					КонецЕсли; 
				КонецЕсли;
...
Процедура ОтправитьФайлТелеграммБоту(ДанныеДляОтправки) Экспорт
	...
	Для каждого ТекУчастник Из ДанныеДляОтправки.ИДДляОтправки Цикл
		...
		Соединение  = КР_РаботаССоциальнымиСетямиКлиентСервер.УстановитьСоединениеСТелеграмм();
		...
		// Разбор ответа
		Если Ответ.КодСостояния <> 200 Тогда
		    ОтветСтрокой   = КР_ОбщийКлиентСервер.СформироватьОтветHTTPВСтруктуру(Ответ);
...
Функция sendDocument(chat_id, ИмяФайлаПолное, AccessToken, ИмяФайла) Экспорт
	...
	Данные.Вставить("ИмяФайла", КР_СерверныеСервер.Транслитерация(ТекущийФайл.ИмяБезРасширения)+"."+ТекущийФайл.Расширение);
...
Функция sendPhoto(chat_id, ИмяФайлаПолное, AccessToken, ИмяФайла) Экспорт
	...
	Данные.Вставить("Boundary", "----" + Строка(Новый УникальныйИдентификатор));
	Данные.Вставить("ИмяФайлаПолное", ИмяФайлаПолное);
	Данные.Вставить("ИмяФайла", КР_СерверныеСервер.Транслитерация(ТекущийФайл.ИмяБезРасширения)+"."+ВтФайл.Расширение);
...
Функция ОтправитьHTTPЗапрос(access_token, method, method_param = Неопределено, Данные = Неопределено) Экспорт
	...
	Попытка
		СоединениеHTTP  = КР_РаботаССоциальнымиСетямиКлиентСервер.УстановитьСоединениеСТелеграмм();
