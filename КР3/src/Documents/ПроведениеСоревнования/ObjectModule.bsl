...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Для Каждого ТекЗнач Из РасчетнаяТаблица Цикл
		...
		НовДвижение.Бонус			= ТекЗнач.Бонус;
		//TODO: не забыть добавить в обновление
	КонецЦикла;
...
Функция ПолучитьРасчетнуюТаблицуРейтинга() Экспорт
	...
	ДанныеПоФормулам = КР_СерверныеСервер.ПолучитьФормулыДляРасчетаРейтинга(Дата);
	
	Для Каждого ТекИгра Из ИгрыСоревнований Цикл
		Если ТекИгра.Выигравший = Справочники.Участники.ПустаяСсылка() Или ТекИгра.Проигравший
			= Справочники.Участники.ПустаяСсылка() Тогда
		...
		ПРв = мКэфВ * РассчитатьПрирост(РейтингВыигравшего, РейтингПроигравшего, , ДанныеПоФормулам);
		...
		ПРп = мКэфП * РассчитатьПрирост(РейтингВыигравшего, РейтингПроигравшего, Ложь, ДанныеПоФормулам);
...
Функция ПолучитьРасчетнуюТаблицуДляБыстрогоРежима() Экспорт
	...
	Запрос.УстановитьПараметр("Внеш", ТаблицаБыстрогоРасчета.Выгрузить());
	Запрос.Выполнить();
...
Функция ПолучитьДетальнуюТаблицуРасчетаКомандные()
	...
	//теперь рассчитаем все приросты игроков
	мРасчетМеждуПолами = Константы.НеРассчитыватьРейтингПриВстречеМужчиныИЖенщины.Получить();
	ФормулыДляРасчета = КР_СерверныеСервер.ПолучитьФормулыДляРасчетаРейтинга(Дата);
	Сч = 1;
	Для Каждого ТекСтрока Из Результат Цикл
		ТекСтрока.ИД = Сч;
		Сч = СЧ + 1;
...
// ДанныеПоФормулам - см.СерверныеСервер.ПолучитьФормулыДляРасчетаРейтинга
Функция РассчитатьПрирост(РТв, РТп, Выигравший = Истина, ДанныеПоФормулам) Экспорт
	...
	Иначе
		...
		Если Выигравший Тогда
			СтрокаФормулы = ДанныеПоФормулам[Перечисления.ТипыПриростов.ПриростВыигравшего];
		Иначе
			СтрокаФормулы = ДанныеПоФормулам[Перечисления.ТипыПриростов.ПриростПроигравшего];			
		КонецЕсли;
		// стрФормула = "ПРв = " + ПолучитьФормулуРейтинга(РТв, РТп, Выигравший);
		стрФормула = "ПРв = " + СтрокаФормулы;
		Попытка
...
Функция НайтиРейтинг(Участник)
	...
	ИскомыйУчастник = СписокУчастников.Найти(Участник, "Участник");
	Если ИскомыйУчастник = Неопределено Тогда
		Возврат 0;
	Иначе
		Возврат ИскомыйУчастник.ТекущийРейтинг;
	КонецЕсли;
