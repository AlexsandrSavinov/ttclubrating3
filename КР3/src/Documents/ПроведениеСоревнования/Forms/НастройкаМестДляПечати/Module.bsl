	...
	Иначе
		Если РежимТура = Перечисления.РежимыТуровСоревнования.Групповой Тогда
			...
			Для Каждого ТекСтр Из ТаблицаГрупп Цикл
				...
				НовСтрока.МестоС 	= ТекМестоС; // 1;
				Если ТекМестоПо = 0 Тогда
					ТекМестоПо = ТекСтр.Количество;
				Иначе
					ТекМестоПо = ТекМестоПо + ТекСтр.Количество - 1;
				КонецЕсли;
				НовСтрока.МестоПо	= ТекМестоПо;
				НовСтрока.КолМестВСетке = ТекСтр.Количество;
				ТекМестоС = ТекМестоПо + 1;
				ТекМестоПо = ТекМестоС;							
			КонецЦикла;
		Иначе
			НовСтрока = ТаблицаМест.Добавить();
			НовСтрока.МестоС = 1;
