...
Процедура ВВыбранные(Команда)
	...
	Иначе
		ТекстСообщения = Строка(Искомый[0].Участник) + " уже есть в выбранных участниках";
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , "ВыбранныеУчастники[" + ВыбранныеУчастники.Индекс(
			Искомый[0]) + "].Участник", , );
	КонецЕсли;
