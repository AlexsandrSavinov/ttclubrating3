...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	АдресКопииТаблицыГрупп = ПоместитьВоВременноеХранилище(ТаблицаГруппы.Выгрузить(),
	...
	Элементы.СписокГруппПереключатели.КоличествоКолонок = ВсегоКолГрупп + 1;
	Заголовок = СсылкаНаСоревнование.НазваниеСоревнования + " " + Параметры.Коммент;
	...
	АдресОбщейТаблицыДанных = ПоместитьВоВременноеХранилище(ТаблицаОбщихДанных.Выгрузить(), УникальныйИдентификатор);
	
	// продумать если 2 групповой, 3 групповой, нужен только 3
	Для Каждого ТекСтрока Из СсылкаНаСоревнование.ХодСоревнования Цикл
		Если ТекСтрока.РежимТура = Перечисления.РежимыТуровСоревнования.Групповой И ТекСтрока.НомерСтроки <> НомерТура Тогда
...
Процедура ПослеЗакрытияФормыВыбораРезультата(Результат, ДопПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		ПроставитьЗначенияПоМониторингу(ТаблицаМониторинга, Результат.Выигравший, Результат.Проигравший,
			Результат.КолПартийВ, Результат.КолПартийП, Ложь, Результат.ТехПоражение);
...
Процедура КомандаБегунки(Команда)
	...
	//TODO: посмотреть и переделать, не выводились
	ТекНомерГруппы = глТекГруппа;
...
Функция ПолучитьКонечныйТабличныйДокумент(ДанныеДляПолучения)
	...
	Для каждого СтрокаРезультата Из РезультатПоНомерамГруппы Цикл
		...
		ИсходныеСтроки = ТЧУчастников.Скопировать(Новый Структура("НомерГруппы,НомерТура,РежимТура", ТекНомерГруппы,
			ДанныеДляПолучения.НомерТура, ДанныеДляПолучения.РежимТура));
		...
		Для каждого ТекСтрокаУчастника Из мИсходныеСтроки Цикл
			...
			ОбластьСтроки.Параметры.Участник	   	= КР_ОбщийКлиентСервер.СформироватьИмяИгрока(ТекСтрокаУчастника.Участник) + 
				" (" + Формат(ТекСтрокаУчастника.РейтингУчастника, "ЧН=0; ЧГ=0;") + ")";
			...
			Для каждого УчастникДляЗаполнения Из мИсходныеСтроки Цикл
				...
				Если ЗначениеЗаполнено(ТекСтрокаУчастника["КолонкаИгры"+НомерКолонкиИгры]) Тогда
					...
					Иначе
						ОбластьДляОчков.Области.ЗначениеКолонкиЯчейка.ЦветТекста = WebЦвета.Черный;
					КонецЕсли; 
	...
	КонецЦикла; 
	 ...
	 ПараметрыВозврата.Вставить("РасчетнаяТаблица", КР_СерверныеСервер.ТаблицаЗначенийВМассив(ИтоговаяИсходныеСтроки));
...
Процедура ОбновитьАдресаТаблиц()
	...
	ДанныеПоАдресам = ОбновитьАдресаТаблицНаСервере(ТаблицаОбщихДанных, ТаблицаГруппы, УникальныйИдентификатор);
...
Процедура ЗаполнитьМониторинг()
	...
	Для каждого СтрТекНомерГруппы Из мНомераГрупп Цикл
		...
		Для каждого ТекИгра Из ТаблицаСПорядковымиИграми Цикл
			...
			Если ПервыйУчастник <> Неопределено И ВторойУчастник <> Неопределено Тогда
				...
				НовСтрока.ПриоритетВызова 	= СчПриоритетаВызова;
				СчПриоритетаВызова 			= СчПриоритетаВызова + 1;
	...
	ТаблицаМониторинга.Сортировать("ПриоритетВызова ВОЗР, НомерГруппы ВОЗР");
...
Процедура ТаблицаМониторингаНомерСтолаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Проверка1_1 = ПроверитьУчастникаНаВызовПоМониторингу(ТаблицаМониторинга.НайтиСтроки(Новый Структура("Участник1",
		ТекСтрокаМониторинга.Участник1)));
	Проверка1_2 = ПроверитьУчастникаНаВызовПоМониторингу(ТаблицаМониторинга.НайтиСтроки(Новый Структура("Участник2",
		ТекСтрокаМониторинга.Участник1)));

	Проверка2_1 = ПроверитьУчастникаНаВызовПоМониторингу(ТаблицаМониторинга.НайтиСтроки(Новый Структура("Участник2",
		ТекСтрокаМониторинга.Участник2)));
	Проверка2_2 = ПроверитьУчастникаНаВызовПоМониторингу(ТаблицаМониторинга.НайтиСтроки(Новый Структура("Участник2",
		ТекСтрокаМониторинга.Участник2)));
...
Процедура ТаблицаМониторингаНомерСтолаНачалоВыбораИзСпискаЗавершение(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйСтол <> Неопределено Тогда
		...
		ТекСтрокаМониторинга.ВремяВызова = КР_СерверныеСервер.ДатаСеанса();
		ДанныеДляВстреч = Новый Структура;
		ДанныеДляВстреч.Вставить("ИДФормы", Строка(УникальныйИдентификатор));
...
Функция ОбновитьКМБыстрогоВвода(парУчастник1,парУчастник2)
	...
	Для М = 0 По МаксимумПартий - 1 Цикл
		мСписок.Добавить("1_" + М, Строка(парУчастник1) + " " + МаксимумПартий + ":" + М + " " + Строка(парУчастник2)
			+ Пробел10);
	КонецЦикла;
	...
	//2 вариант
	Для М = 0 По МаксимумПартий - 1 Цикл
		мСписок.Добавить("2_" + М, Строка(парУчастник2) + " " + МаксимумПартий + ":" + М + " " + Строка(парУчастник1)
			+ Пробел10);
	КонецЦикла;
		...
		Для мСтол = 1 По КолСтолов Цикл
			...
			Если ИскомаяИграВМониторинге.Количество() > 0 Тогда // игра есть
				...
				Если мПар2 > мПар1 Тогда
					...
					ОбластьСтола.Параметры.Участник1		= КР_ОбщийКлиентСервер.СформироватьИмяИгрока(
						ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИскомаяИграВМониторинге[0].Участник1, "Наименование"));
					ОбластьСтола.Параметры.НомерУч2			= мПар2;
					ОбластьСтола.Параметры.Участник2			= КР_ОбщийКлиентСервер.СформироватьИмяИгрока(
						ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИскомаяИграВМониторинге[0].Участник2, "Наименование"));
				Иначе
...
Процедура ТабличныйДокументГруппОбработкаДополнительнойРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка, ДополнительныеПараметры)
	 ...
	 Если СтрНайти(Расшифровка, "#") > 0 Тогда
		 ...
		 Если ТекЗначение = "2" Или ТекЗначение = "" Тогда //по этой строке выигравший
			 ...
			 РейтингВ			= ДанныеРасшифровки.Рейтинг1;
			 НомерВ				= НомерУч1Расшифровки;
			 ПроигравшийИгрок 	= ИскомыйУчастник2;
			 РейтингП			= ДанныеРасшифровки.Рейтинг2;
		 ...
		 ИначеЕсли ТекЗначение = "1" Или ТекЗначение = "0" Тогда //тогда эта строка проигравший
			 ...
			 РейтингП			= ДанныеРасшифровки.Рейтинг1;
			 НомерП				= НомерУч1Расшифровки;
			 ВыигравшийИгрок  	= ИскомыйУчастник2;
			 РейтингВ			= ДанныеРасшифровки.Рейтинг2;
		 ...
		 ДляЗаполненияРезультата.РейтингВыигравший	= РейтингВ;
		 ДляЗаполненияРезультата.РейтингПроигравший = РейтингП;
...
Функция РазобратьРасшифровку(парРасшифровка, РасчетнаяТаблица)
	...
	ИскомыйУчастникСтроки1 = РасчетнаяТаблица.НайтиСтроки(Новый Структура("НомерУчастника,НомерГруппы", НомерУч1Расшифровки, ГруппаРасшифровки));
	Если ИскомыйУчастникСтроки1.Количество() > 0 Тогда
		ИскомыйУчастник1 	= ИскомыйУчастникСтроки1[0].Участник;
		Рейтинг1			= ИскомыйУчастникСтроки1[0].РейтингУчастника;
	Иначе
		ИскомыйУчастник1 = ПредопределенноеЗначение("Справочник.Участники.ПустаяСсылка");
		Рейтинг1 = 0;
	КонецЕсли; 
	
	ИскомыйУчастникСтроки2 = РасчетнаяТаблица.НайтиСтроки(Новый Структура("НомерУчастника,НомерГруппы", НомерУч2Расшифровки, ГруппаРасшифровки));
	Если ИскомыйУчастникСтроки2.Количество() > 0 Тогда
		ИскомыйУчастник2 	= ИскомыйУчастникСтроки2[0].Участник;
		Рейтинг2			= ИскомыйУчастникСтроки2[0].РейтингУчастника;
	Иначе
	...
	ВозвратДанных.Вставить("Рейтинг1", Рейтинг1);
	ВозвратДанных.Вставить("Участник2", ИскомыйУчастник2);
	ВозвратДанных.Вставить("Рейтинг2", Рейтинг2);
...
Процедура ТабличныйДокументГруппПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	ВведенноеЗначение 	= Область.Текст;
	Если ВведенноеЗначение = "" Тогда
		Возврат;
	КонецЕсли;
	...
	СколькоДолжноБытьПартий = КР_ОбщийКлиентСервер.ОпределитьКолПартий(КолПартий);
	//участники
	...
	Иначе
		...
		Если ВведенноеЗначение >= СколькоДолжноБытьПартий Или -ВведенноеЗначение >= СколькоДолжноБытьПартий Тогда
			ДопКол = "";
			...
			ИначеЕсли КолПартий = 7 Тогда
				ДопКол = "0,1,2 или 3";
			КонецЕсли;
			ПоказатьПредупреждение(Новый ОписаниеОповещения("ПослеВводаЗначенияВТабДок", ЭтотОбъект, Область),
				"Количество проигранных партий может быть равно " + ДопКол);
			Возврат;
		КонецЕсли;
	...
	Если ИскомыеСтроки.Количество() > 0 Тогда //строки уже есть перезаполняем
		Для Каждого ТекСтрокаИскомая Из ИскомыеСтроки Цикл
			ТаблицаОбщихДанных.Удалить(ТаблицаОбщихДанных.Индекс(ТекСтрокаИскомая));
		КонецЦикла;
	ИначеЕсли ИскомыеСтроки.Количество() = 0 Тогда
		// попробуем другой вариант вдруг выигравший изменился
		Отбор = Новый Структура;                               
	...
	ПроставитьЗначенияПоМониторингу(ТаблицаМониторинга, ВыигравшийУчастник, ПроигравшийУчастник, ИтогВ, ИтогП, ТехВ,
		ТехП);
	ВывестиАктивностьСтолов();
	ОбновитьАдресаТаблиц();
	ЗаполнитьТаблицуПоНомеруГруппы(СписокГруппПереключатели);
...
Процедура СписокГруппПереключателиПриИзменении(Элемент)
	...
	Элементы.ТаблицаМониторингаНомерГруппы.Видимость = СписокГруппПереключатели = 99;
...
Процедура ТабличныйДокументГруппПриАктивизации(Элемент)
	...
	Попытка
		...
		КонецЕсли; 
	Исключение
		КР_ОбщийКлиентСервер.СообщитьПользователю(ОписаниеОшибки(), "", Неопределено);
	КонецПопытки; 
