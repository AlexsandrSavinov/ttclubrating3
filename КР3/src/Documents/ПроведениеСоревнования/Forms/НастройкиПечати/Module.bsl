...
#Область ОбработчикиСобытийЭлементовТаблицыФормыДокОбъектХодСоревнования

&НаКлиенте
Процедура ДокОбъектХодСоревнованияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	Если Поле.Имя = "ДокОбъектХодСоревнованияНастройкиМестJSON" Тогда
		ТекСтрока = ДокОбъект.ХодСоревнования.НайтиПоИдентификатору(ВыбраннаяСтрока);
		Если ТекСтрока <> Неопределено Тогда
			ОповещениеОЗакрытии = Новый ОписаниеОповещения("ПослеНастройкиМест", ЭтотОбъект, ВыбраннаяСтрока);
			ОткрытьФорму("Документ.ПроведениеСоревнования.Форма.НастройкаМестДляПечати",
				ПолучитьПараметрыДляЗаполненияМест(ВыбраннаяСтрока), ЭтотОбъект, УникальныйИдентификатор, , ,
				ОповещениеОЗакрытии);
		КонецЕсли;
	КонецЕсли;
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыДокОбъектХодСоревнованияКоманды

&НаКлиенте
Процедура ДокОбъектХодСоревнованияКомандыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, ЭтоГруппа, Параметр)
	УстановитьОтказ(Отказ);
КонецПроцедуры

&НаКлиенте
Процедура ДокОбъектХодСоревнованияКомандыПередУдалением(Элемент, Отказ)
	УстановитьОтказ(Отказ);
КонецПроцедуры
...
// 
// Параметры:
//  СтрИсточник - ДанныеФормыЭлементКоллекции, СтрокаТаблицыЗначений - Стр источник
//  СтрПриемник - ДанныеФормыЭлементКоллекции - Стр приемник
&НаСервере
...
Процедура ПечатьСНастройкамиНаКлиенте()
	...
	Если ДокОбъект.ОбщийРежимСоревнования = 0 Тогда // ЛИЧНЫЕ
		...
		Для Каждого ТекСтрока Из ДокОбъект.ХодСоревнования Цикл
			...
			Сч = Сч + 1;
			Состояние("Печать протоколов соревнований", Сч * 100 / МаксВсегоСтрок, ТекСтрока.Комментарий,
			...
			Если ТекСтрока.РежимТура = ПредопределенноеЗначение("Перечисление.РежимыТуровСоревнования.Групповой") Тогда
				...
				Данные.Вставить("НастройкиМест", ТекСтрока.НастройкиМестJSON);
				Данные.Вставить("ПромежуточныеПротоколы", ПромежуточныеПротоколы);
				ТабДокумент = ФормированиеТаблицГрупп(Данные, СИграми);
				КР_ОбщийКлиентСервер.ЗадатьПараметрыПечатиДляПротокола(ТабДокумент, ТекСтрока.РежимТура);
				Если ВыводитьВОдинЛист Тогда
				...
				Иначе
					ОткрытьФорму("ОбщаяФорма.ФормаВыводаНаПечать", Новый Структура("ТабДок,Заголовок,Ссылка",
						ТабДокумент, ТекСтрока.Комментарий, ДокОбъект.Ссылка), ЭтотОбъект,
			...
			Иначе
				...
				Форма = ПолучитьФорму("Документ.ПроведениеСоревнования.Форма.ФормаМинус2ИОлимпийка", ПараметрыФормы,
					ЭтотОбъект);
				ТабДокумент = Форма.ТабДок;
				Если ПромежуточныеПротоколы И ЗначениеЗаполнено(ТекСтрока.НастройкиМестJSON) Тогда
					МассивНастроек = КоннекторHTTP.JSONВЗначение(ТекСтрока.НастройкиМестJSON);
					Если МассивНастроек.Количество() > 0 Тогда
						СеткаНа 		= ТекСтрока.СеткаНа;
						УстанавливЗнач 	= МассивНастроек[0].МестоС;
						Для Н = 1 По СеткаНа Цикл
							Для М = 1 По 2 Цикл
								...
								Если ИскомаяОбласть <> Неопределено Тогда
									Если УстанавливЗнач > МассивНастроек[0].МестоПо Тогда
		...
		Для Каждого ТекСтрока Из ТаблицаЭтапов Цикл
			...
			КонецЕсли;
			Сч = Сч + 1;
			Состояние("Печать протоколов соревнований", Сч * 100 / МаксВсегоСтрок, "Итоговые места: "
				+ ТекСтрока.Комментарий, БиблиотекаКартинок.Информация);
			Данные = Новый Структура;
			Данные.Вставить("НомерСтроки", ТекСтрока.НомерСтроки);
			Данные.Вставить("Комментарий", ТекСтрока.Комментарий);
			...
			Если ВыводитьВОдинЛист Тогда
				ВывестиТабДокВОсновной(глТабДок, ТабДокумент);
			Иначе
				ОткрытьФорму("ОбщаяФорма.ФормаВыводаНаПечать", Новый Структура("ТабДок,Заголовок,Ссылка", ТабДокумент,
					ТекСтрока.Комментарий, ДокОбъект.Ссылка), ЭтотОбъект, Новый УникальныйИдентификатор);
			КонецЕсли;
		КонецЦикла;
	Иначе //КОМАНДНЫЕ
		мПеремПоХоду = 0;
		...
		Для Каждого ТекСтрока Из ДокОбъект.ХодСоревнованияКоманды Цикл
			...
			Иначе
				...
				ПараметрыФормы.Вставить("Коммент", ТекСтрока.Комментарий);
				ПараметрыФормы.Вставить("КолПартий", ТекСтрока.КолПартий);
				ПараметрыФормы.Вставить("ВидСоревнований", ПредопределенноеЗначение(
					"Перечисление.ВидыСоревнований.Командные"));
				ПараметрыФормы.Вставить("ЖеребьевкаКоманд", ТекСтрока.Жеребьевка);
				Форма = ПолучитьФорму("Документ.ПроведениеСоревнования.Форма.ФормаМинус2ИОлимпийкаКоманды",
		...
		Если СписокКоманд Тогда
			...
			Иначе
				ОткрытьФорму("ОбщаяФорма.ФормаВыводаНаПечать", Новый Структура("ТабДок,Заголовок,Ссылка",
					ОсновнойДокПолногоПротокола, ДокОбъект.НазваниеСоревнования, ДокОбъект.Ссылка), ЭтотОбъект,
					Новый УникальныйИдентификатор);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ВыводитьПредварительныйРейтинг Тогда
		Сч = Сч + 1;
		Состояние("Печать протоколов соревнований", Сч * 100 / МаксВсегоСтрок, "Предварительный рейтинг",
			БиблиотекаКартинок.Информация);
		...
		Иначе
			ОткрытьФорму("ОбщаяФорма.ФормаВыводаНаПечать", Новый Структура("ТабДок,Заголовок,Ссылка", ТабДокумент,
				"Предварительный рейтинг", ДокОбъект.Ссылка), ЭтотОбъект, Новый УникальныйИдентификатор);
		КонецЕсли;
	КонецЕсли;
	Если ВыводитьВОдинЛист Тогда
		КР_ОбщийКлиентСервер.ЗадатьПараметрыПечатиДляПротокола(глТабДок, ПредопределенноеЗначение(
			"Перечисление.РежимыТуровСоревнования.МинусДва"));
		ОткрытьФорму("ОбщаяФорма.ФормаВыводаНаПечать", Новый Структура("ТабДок,Заголовок,Ссылка", глТабДок,
			ДокОбъект.НазваниеСоревнования, ДокОбъект.Ссылка), ЭтотОбъект, Новый УникальныйИдентификатор);
	КонецЕсли;
...
Функция ФормированиеТаблицГруппПоМестам(ТекСтр)
	...
	Для Н = 1 По КоличествоГруппПоТуру Цикл
		...
		Для Каждого ТекДанныеТаблицы Из ДанныеПоМестамУчастников Цикл
			...
			Иначе
				ОбластьСтроки.Параметры.Участник	   = КР_ОбщийКлиентСервер.СформироватьИмяИгрока(ТекДанныеТаблицы["Участник"]);
			КонецЕсли; 
...
Функция ПолучитьТабличныйДокументПоМестамСеток(Данные)
	...
	Для Каждого ТекДанныеТаблицы Из ДанныеПоМестамУчастников Цикл
		...
		Иначе
			ОбластьСтроки.Параметры.Участник	   = КР_ОбщийКлиентСервер.СформироватьИмяИгрока(ТекДанныеТаблицы.РеквизитУчастник);
		КонецЕсли;
		ТабДок.Вывести(ОбластьСтроки);
		//рейтинг
		//место
...
Процедура ПромежуточныеПротоколыПриИзменении(Элемент)
	...
	Элементы.ДокОбъектХодСоревнованияНастройкиМестJSON.Видимость = ПромежуточныеПротоколы;
	
КонецПроцедуры

&НаСервере
Функция ПолучитьПараметрыДляЗаполненияМест(ИдентификаторСтроки)

	ПараметрыНастройкиМест = Новый Структура;
	
	ТекСтрока = ДокОбъект.ХодСоревнования.НайтиПоИдентификатору(ИдентификаторСтроки);
	
	ПараметрыНастройкиМест.Вставить("НастройкиМестJSON", ТекСтрока.НастройкиМестJSON);
	ПараметрыНастройкиМест.Вставить("РежимТура", ТекСтрока.РежимТура);
	ПараметрыНастройкиМест.Вставить("НомерТура", ТекСтрока.НомерСтроки);
	ПараметрыНастройкиМест.Вставить("СеткаНа", ТекСтрока.СеткаНа);
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц();
	Запрос.Текст = "ВЫБРАТЬ * ПОМЕСТИТЬ втТЗ ИЗ &Внеш КАК Внеш";
	Запрос.УстановитьПараметр("Внеш", ДокОбъект.УчастникиПоЖеребьевке.Выгрузить());
	Запрос.Выполнить();
	Запрос.УстановитьПараметр("РежимТура", ПараметрыНастройкиМест.РежимТура);
	Запрос.УстановитьПараметр("НомерТура", ПараметрыНастройкиМест.НомерТура);
	Запрос.Текст = "ВЫБРАТЬ
	|	ПроведениеСоревнованияУчастникиПоЖеребьевке.НомерГруппы КАК НомерГруппы,
	|	КОЛИЧЕСТВО(ПроведениеСоревнованияУчастникиПоЖеребьевке.Участник) КАК Количество
	|ИЗ
	|	втТЗ КАК ПроведениеСоревнованияУчастникиПоЖеребьевке
	|ГДЕ
	|	ПроведениеСоревнованияУчастникиПоЖеребьевке.РежимТура = &РежимТура
	...
	ПараметрыНастройкиМест.Вставить("МассивГрупп", ОбщегоНазначения.ТаблицаЗначенийВМассив(РезультатЗапроса));
	
	Возврат ПараметрыНастройкиМест;
	
КонецФункции

&НаКлиенте
Процедура ПослеНастройкиМест(Результат, ДопПараметры) Экспорт
	
	Если Результат <> Неопределено Тогда
		ТекСтрока = ДокОбъект.ХодСоревнования.НайтиПоИдентификатору(ДопПараметры);
		ТекСтрока.НастройкиМестJSON = Результат;
	КонецЕсли;
