	...
	Иначе
		Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда // это копирование
			...
			Для каждого ТекСтр Из Объект.СписокУчастников Цикл
				ТекСтр.ТекущийРейтинг = КР_СерверныеСервер.ПолучитьТекущийРейтингУчастника(ТекСтр.Участник, ТекущаяДатаСеанса(), Объект.ВидРейтинга);
			КонецЦикла;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не ЗавершениеРаботы Тогда
		КР_СерверныеСервер.УдалитьВсеТекущиеВстречи(УникальныйИдентификатор);
	КонецЕсли; 
...
Процедура ХодСоревнованияКомандыКолПартийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПоказатьВыборИзСписка(Новый ОписаниеОповещения("ХодСоревнованияКомандыКолПартийНачалоВыбораЗавершение", ЭтотОбъект), Список);
...
Процедура ПодборУчастников(Команда)
	...
	ОткрытьФорму("Документ.ПроведениеСоревнования.Форма.ПодборУчастников",ПараметрыФормы,ЭтотОбъект,Новый УникальныйИдентификатор,,,,КР_СерверныеСервер.ПолучитьРежимОткрытияФормыПодбора());
КонецПроцедуры
...
Процедура КомандаОбновитьРейтинг(Команда)
	мТекДата = КР_СерверныеСервер.ДатаСеанса();
...
Процедура ОбновитьКнопкуПроведения()
	КнопкаИзменения = Команднаяпанель.ПодчиненныеЭлементы.Найти("ФормаПровестиИЗакрыть");
		...
		Если ИсточникВыбора.Выбран Тогда
			...
			Иначе
				...
				Если ИсточникВыбора.ИсточникВызова = 1 Тогда
					...
					ТекущийЭлемент = Элементы.ТаблицаБыстрогоРасчетаКолПартий1;
				Иначе
					...
					ТекущийЭлемент = Элементы.ТаблицаБыстрогоРасчетаУчастник2;
				КонецЕсли; 
				Если ПроверитьЗаполненностьУчастниковВСтроке() Тогда
					ПерерасчитатьПриростыИИтоговыйРейтинг();
				КонецЕсли;
		...
		Если втТекКоманда.Пометка Тогда
			...
			Для Каждого втТекУчастник Из СтрокиСУчастниками Цикл
				Если втТекУчастник.Пометка Тогда //добавляем в таблицу команд
					...
					НовСтрока.ТекущийРейтинг		= КР_СерверныеСервер.ПолучитьТекущийРейтингУчастника(НовСтрока.Участник, 
						КР_СерверныеСервер.ДатаСеанса(), Объект.ВидРейтинга);
				КонецЕсли; 
			КонецЦикла; 
		КонецЕсли; 	
	КонецЦикла;
...
Процедура ЗаполнитьТаблицуДанныхИзТабличногоДокумента(ДанныеДляЭкспорта,ДанныеЗагрузки)
	...
	Для Сч = 2 По КоличествоСтрок Цикл
		...
		КонецЕсли;
		ИтоговыйПарДаты = КР_ОбщийКлиентСервер.ПриведениеИзСтрокиВДату(парДатаРождения);
...
Процедура ДобавитьВСписокЭкспортнуюТаблицу(ДанныеЭкспорта,ТаблицаЭкспорта)
	...
	Для Каждого ТекЗначение Из ТаблицаЭкспорта Цикл
		...
		Если ДанныеЭкспорта.ЗагружатьИЗФайлаРейтинг = 1 Тогда
			мРейтинг = КР_СерверныеСервер.ПолучитьТекущийРейтингУчастника(Участник, 
				КР_СерверныеСервер.ДатаСеанса(), Объект.ВидРейтинга);
		Иначе
...
Процедура ПерерасчитатьПриростыИИтоговыйРейтинг(ИДСтрокиПерерасчета = Неопределено)
	...
	ДанныеПоФормулам = КР_СерверныеСервер.ПолучитьФормулыДляРасчетаРейтинга(Объект.Дата);
...
Процедура ОбновитьРейтингВСпискеУчастников(мДатаОбновления = Неопределено)
	Для Каждого ТекСтр Из Объект.СписокУчастников Цикл
		ТекСтр.ТекущийРейтинг = КР_СерверныеСервер.ПолучитьТекущийРейтингУчастника(ТекСтр.Участник, ?(мДатаОбновления
...
Функция ВернутьАдресСпискаУчастников()
	Возврат ПоместитьВоВременноеХранилище(Объект.СписокУчастников.Выгрузить(),УникальныйИдентификатор);
КонецФункции
