...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПервыйУчастник 	= КР_ОбщийКлиентСервер.СформироватьИмяИгрока(ТаблицаПартий.Получить(0).Участник);
	ВторойУчастник	= КР_ОбщийКлиентСервер.СформироватьИмяИгрока(ТаблицаПартий.Получить(1).Участник);
	
	Заголовок = "Результат встречи: " + ПервыйУчастник + " - " + ВторойУчастник;
	ЗаголовокКолонки1 = Строка(ПервыйУчастник) + " (" + Формат(ТаблицаПартий.Получить(0).Рейтинг, "ЧГ=0;") + ")";
	ЗаголовокКолонки2 = Строка(ВторойУчастник) + " (" + Формат(ТаблицаПартий.Получить(1).Рейтинг, "ЧГ=0;") + ")";
	
	Элементы.ТаблицаВыбораПоИтогамКолПартий1.Заголовок = ЗаголовокКолонки1;
	Элементы.ТаблицаВыбораПоИтогамКолПартий2.Заголовок = ЗаголовокКолонки2;
...
Процедура ВариантыВыбораКоличестваПартийПриИзменении(Элемент)
	ВариантыВыбораКоличестваПартийПриИзмененииНаСервере();
	ИтогСтроки = КР_ОбщийКлиентСервер.ОпределитьКолПартий(ВариантыВыбораКоличестваПартий);
	...
	Иначе //ввели минусовое значение
		...
		Если втТекЗнач >=0 Тогда
			...
			Иначе
				ПоказатьПредупреждение(Новый ОписаниеОповещения("ТаблицаПартийПриИзмененииЗавершение", ЭтотОбъект,
					Новый Структура("НомГруппы, НомКолонки, СтрокаДляУстановки, ТекЗначение, ТекСтрока", НомерГруппы,
					НомКолонки, СтрокаДляУстановки, ТекЗначение, ТекСтрока)),
					"Проверьте правильность введенных очков !");
				Возврат;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура Выбрать(Команда)
	...
	Иначе
		Если ПроверитьКорректноеЗаполнениеПоОчкам() Тогда
			Закрыть(ПодготовитьЗаполненныеДанные());
		КонецЕсли; 
	КонецЕсли; 
	...
	Если ВыбраннаяСтрока <> Неопределено Тогда
		...
		Если ТипЗнч(ВыбраннаяСтрока.КолПартий1) = Тип("Строка") И ТипЗнч(ВыбраннаяСтрока.КолПартий2) = Тип("Строка") Тогда //это техническое 
			Если ВыбраннаяСтрока.КолПартий1 = "W" И ВыбраннаяСтрока.КолПартий2 = "L" Тогда
				...
				КолПартийВ		= КР_ОбщийКлиентСервер.ОпределитьКолПартий(ВариантыВыбораКоличестваПартий);
				КолПартийП		= 0;
		...
		СтруктураВозврата.Вставить("ТаблицаОчков",КР_СерверныеСервер.ТаблицаЗначенийВМассив(ТаблицаПартий.Выгрузить()));
