...
// Получить таблицу игр тура.
// 
// Параметры:
//  КолУчастников - Число - Кол участников
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Получить таблицу игр тура:
// * НомерТура - Число 
// * Первый - Число 
// * Второй - Число 
...
Функция ФормированиеТаблицГрупп(ТекСтр,СИграми = Ложь,ДокОбъект, ВыводитьПолноеФИО = Ложь) Экспорт
	...
	Для Н = 1 По КоличествоГруппПоТуру Цикл
		...
		Для К = 0 По КоличествоУчастников.ВГраница() Цикл
			...
			ОбластьСтроки.Параметры.Участник	   	= КР_ОбщийКлиентСервер.СформироватьИмяИгрока(КоличествоУчастников[К].Участник, ВыводитьПолноеФИО) +
	...
	Если ЗначениеЗаполнено(ДокОбъект.ГлавныйСудья) Или ЗначениеЗаполнено(ДокОбъект.Секретарь) Тогда
		...
		Если ЗначениеЗаполнено(ДокОбъект.ГлавныйСудья) Тогда
			Подвал.Параметры.глСудья =  КР_ОбщийКлиентСервер.СформироватьИмяИгрока(ДокОбъект.ГлавныйСудья);
		КонецЕсли;
		Если ЗначениеЗаполнено(ДокОбъект.Секретарь) Тогда
			Подвал.Параметры.Секретарь =  КР_ОбщийКлиентСервер.СформироватьИмяИгрока(ДокОбъект.Секретарь);
		КонецЕсли;
...
Функция ТабличныйДокументРейтинг(ДокСсылка, ВыводитьПолноеФИО = Ложь) Экспорт
	...
	Для Каждого ТекСтрока Из ТаблицаРасчета Цикл
		...
		СтрокаТаблицы.Параметры.Участник = КР_ОбщийКлиентСервер.СформироватьИмяИгрока(ТекСтрока.Участник, ВыводитьПолноеФИО);
...
Функция ТабличныйДокументСписокКоманд(ДокСсылка) Экспорт
	...
	Если ЗначениеЗаполнено(ДокСсылка.ГлавныйСудья) Тогда
		ОбластьПодвал.Параметры.глСудья = КР_ОбщийКлиентСервер.СформироватьИмяИгрока(ДокСсылка.ГлавныйСудья);
	КонецЕсли;
	Если ЗначениеЗаполнено(ДокСсылка.Секретарь) Тогда
		ОбластьПодвал.Параметры.Секретарь = КР_ОбщийКлиентСервер.СформироватьИмяИгрока(ДокСсылка.Секретарь);
	КонецЕсли;
		...
		Для каждого ТекГруппаВывода Из ТаблицаКомандПоТуру Цикл
			...
			Если ОбщиеДанные.Количество() > 0 Тогда
				...
				Для Каждого ТекСтрИгр Из втОбщиеСыгранныеИгрыПоГруппе Цикл //вывод табличек игр
					глВсегоПартийДляПобеды = КР_ОбщийКлиентСервер.ОпределитьКолПартий(ТекТур.КолПартий);
