...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		//ТекущийРейтинг = КР_СерверныеСервер.ПолучитьТекущийРейтингУчастника(Объект.Ссылка,ТекущаяДата());
		ТекущийРазряд = КР_СерверныеСервер.ПолучитьРазрядЗваниеУчастника(Объект.Ссылка,ТекущаяДата());
...
Процедура НаименованиеПриИзменении(Элемент)
	...
	Фамилия 	 = КР_ОбщийКлиентСервер.ВыделитьСлово(ФИО);
	Имя     		 = КР_ОбщийКлиентСервер.ВыделитьСлово(ФИО);
	Отчество 	 = КР_ОбщийКлиентСервер.ВыделитьСлово(ФИО);
...
Процедура СсылкаНаКартинкуНажатие(Элемент, СтандартнаяОбработка)
	...
	Диалог.Показать(Новый ОписаниеОповещения("СсылкаНаКартинкуНажатиеЗавершение1", ЭтотОбъект,
		Новый Структура("АдресКартинки, Диалог", АдресКартинки, Диалог)));
...
Процедура ТекущийРейтингНажатие(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ФормаДока = ПолучитьФорму("Документ.КорректировкаРейтинга.ФормаОбъекта");
